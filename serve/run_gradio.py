...
LLM_MODEL_DICT = {
    ...
    "wenxin": ["ernie-3.5", "ernie-3.5-8k", "ernie-lite","ernie-4.0"],
    "xinhuo": ["Spark Lite","Spark Pro","Spark Max","Spark4.0 Ultra"],
    "zhipuai": ["glm-4-plus", "glm-4-airx"]
}
...
CHAT_MODE_LIST = ['chat with llm','Retrieval-augmented generation']
INIT_MODE = 'chat with llm'
...
class Model_center():
    ...
    def chat_qa_chain_self_answer(self, question: str, chat_history: list = [], model: str = "openai", embedding: str = "openai", temperature: float = 0.0, top_k: int = 4, history_len: int = 3, file_path: str = DEFAULT_DB_PATH, persist_path: str = DEFAULT_PERSIST_PATH):
        ...
        try:
            ...
            return "", chain.answer(question=question, temperature=temperature, top_k=top_k,history_len=history_len)
...
def handle_respond(msg, chat_history_llm, chat_history_rag,llm, embeddings, temperature, top_k, history_len,chat_mode):

    if chat_mode == "chat with llm":
        msg, llm_response = respond(msg,chat_history_llm,llm,history_len,temperature)
        RAG_response = gr.update(visible=False)
        return msg,llm_response,RAG_response
    else:
        msg,RAG_response = model_center.chat_qa_chain_self_answer(msg,chat_history_rag,llm,embeddings,temperature,top_k,history_len)
        llm_response = gr.update(visible=False)
        return msg,llm_response,RAG_response
 


# 控制不同模式下显示的 chatbot
def update_chatbot_visibility(mode):
        # 如果用户选择 "RAG" 模式，显示 RAG Chatbot，隐藏 LLM Chatbot
        if mode == "Retrieval-augmented generation":
            return gr.update(visible=True), gr.update(visible=False)
        # 如果用户选择 "Chat with LLM" 模式，显示 LLM Chatbot，隐藏 RAG Chatbot
        else:
            return gr.update(visible=False), gr.update(visible=True)

def clear_chatbot(chat_mode):
    if chat_mode == "chat with llm":
...
with block as demo:
    with gr.Row(equal_height=True):           
        ...
        with gr.Column(scale=2):
            gr.Markdown("""<h1><center>LLM is the future</center></h1>
                """)
        gr.Image(value=DATAWHALE_LOGO_PATH, scale=1, min_width=10, show_label=False, show_download_button=False, container=False)

    with gr.Row():
        with gr.Column(scale=4):
            llm_chatbot = gr.Chatbot(label="LLM Chatbot",visible=True,height=600, show_copy_button=True, show_share_button=True, avatar_images=(AIGC_AVATAR_PATH, DATAWHALE_AVATAR_PATH))
            RAG_chatbot = gr.Chatbot(label="RAG Chatbot",visible=False,height=600, show_copy_button=True, show_share_button=True, avatar_images=(AIGC_AVATAR_PATH, DATAWHALE_AVATAR_PATH))
            # 创建一个文本框组件，用于输入 prompt。
            msg = gr.Textbox(label="Prompt/问题")

            with gr.Row():
                # 创建一个清除按钮，用于清除聊天机器人组件的内容。
                clear_button = gr.ClearButton(value="Clear console")
        ...
        with gr.Column(scale=1):
         ...
         # 绑定下拉框的选择事件，用于切换显示的 Chatbot
        chat_mode.change(update_chatbot_visibility, 
                        inputs=[chat_mode], 
                        outputs=[RAG_chatbot, llm_chatbot])
        ...
        msg.submit(handle_respond, inputs=[msg,llm_chatbot,RAG_chatbot,llm, embeddings,temperature,top_k,history_len,chat_mode], outputs=[msg,llm_chatbot,RAG_chatbot],show_progress=True)#if chat_mode == "Retrieval-augmented generation" else llm_chatbot
        # 点击后清空后端存储的聊天记录
        clear_button.click(clear_chatbot,inputs=[chat_mode], outputs=[llm_chatbot,RAG_chatbot])
