...
class Spark_LLM(Self_LLM):
    ...
    def _call(self, prompt : str, stop: Optional[List[str]] = None,
        ...
        self.url, self.domain = get_url_domain_spark(model=self.model_name)
        # 将 Prompt 填充到星火格式
        # question = self.getText("user", prompt)
        # 发起请求
        try:
            response = spark_main(self.appid,self.api_key,self.api_secret,self.url,self.domain,prompt, self.temperature, self.max_tokens)
            return response
...
# 收到websocket消息的处理
def on_message(ws, message, output_queue):
    # print(message)
...
def spark_main(appid, api_key, api_secret, Spark_url,domain, query, temperature, max_tokens):
    # print("星火:")
    ...
    ws = websocket.WebSocketApp(wsUrl, on_message = lambda ws, message: on_message(ws, message, output_queue), on_error=on_error, on_close=on_close, on_open=on_open)
    ws.appid = appid
    ws.query = query
    ws.domain = domain
